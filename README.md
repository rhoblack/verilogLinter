...
- **향상된 자동 완성 (Smart Completion)**: 워크스페이스 내의 모듈 인스턴스와 더불어, 사용자가 작성 중인 파일에서 선언한 다양한 변수(포트, 와이어, 로직, 루프 이터레이터 등)를 똑똑하게 추출하여 자동 완성을 제안합니다.
- **스마트 괄호 자동 닫기 및 주석 토글**: `()`, `{}`, `[]`, `""`, `''`, `/* */` 등을 입력할 때 반대쪽 짝을 자동으로 완성해 주며, 단축키(`Ctrl + /`, `Shift + Alt + A`)를 통한 줄/블록 주석 토글을 완벽 지원합니다.
- **대응 짝 키워드 하이라이트 (Matching Highlights)**: `begin - end`, `module - endmodule`, `task - endtask`, `function - endfunction`, `class - endclass` 등 서로 짝이 되는 주요 블록 키워드들에 마우스 커서를 두면, 한 쌍으로 이어진 상대편 키워드를 자동으로 굵게 강조하고 박스 표시해 구조를 쉽게 파악할 수 있게 돕습니다.
